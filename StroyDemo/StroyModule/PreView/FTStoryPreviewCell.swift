...
protocol FTStoryPreviewProtocol: class {
...
class FTStoryPreviewCell: UICollectionViewCell , UIScrollViewDelegate {
    
    public weak var delegate : FTStoryPreviewProtocol?{
        didSet{
            storyHeaderView.delegate = self
        }
    }
    
    private lazy var storyHeaderView: FTStroyHeaderView = {
        let v = FTStroyHeaderView()
    ...
    private var snapViewXPos: CGFloat {
        return (mediaIndex == 0) ? 0 : scrollview.subviews[previousSnapIndex].frame.maxX
    }
    private var videoMediaIndex: Int = 0
    private var handpickedMediaIndex: Int = 0
    var retryBtn : FTRetryLoaderButton!
    ...
    public var mediaIndex: Int = 0 {
        didSet {
            scrollview.isUserInteractionEnabled = true
            switch direction {
                case .forward:
                    if mediaIndex < story?.mediaCount ?? 0 {
                        if let media = story?.mediaList?[mediaIndex] {
                            if media.type != MediaType.video {
                                if let snapView = getSnapview() {
                                    startRequest(snapView: snapView, with: media.url ?? "")
                                } else {
                                    let snapView = createSnapView()
                                    startRequest(snapView: snapView, with: media.url ?? "")
                                }
                            }else {
                                if let videoView = getVideoView(with: mediaIndex) {
                                    startPlayer(videoView: videoView, with: media.url ?? "")
                                }else {
                ...
                case .backward:
                    if mediaIndex < story?.mediaCount ?? 0 {
                        if let media = story?.mediaList?[mediaIndex] {
                            if media.type != MediaType.video {
                                if let snapView = getSnapview() {
                                    self.startRequest(snapView: snapView, with: media.url ?? "")
                                }
                            }else {
                                if let videoView = getVideoView(with: mediaIndex) {
                                    startPlayer(videoView: videoView, with: media.url ?? "")
                                    ...
                                    self.startPlayer(videoView: videoView, with: media.url ?? "")
                                }
                            }
                        }
                }
    ...
    private func createSnapView() -> UIImageView {
        ...
        snapView.tag = mediaIndex + snapViewTagIndicator
         ...
         */
        scrollview.subviews.filter({$0.tag == mediaIndex + snapViewTagIndicator}).first?.removeFromSuperview()
        
        scrollview.addSubview(snapView)
        
        /// Setting constraints for snap view.
        NSLayoutConstraint.activate([
            snapView.leadingAnchor.constraint(equalTo: (mediaIndex == 0) ? scrollview.leadingAnchor : scrollview.subviews[previousSnapIndex].trailingAnchor),
        ...
        if(mediaIndex != 0) {
            NSLayoutConstraint.activate([
                snapView.leadingAnchor.constraint(equalTo: scrollview.leadingAnchor, constant: CGFloat(mediaIndex)*scrollview.width)
            ])
        }
        return snapView
    }
    private func getSnapview() -> UIImageView? {
        if let imageView = scrollview.subviews.filter({$0.tag == mediaIndex + snapViewTagIndicator}).first as? UIImageView {
            return imageView
        }
        return nil
    }
    private func createVideoView() -> FTStroyPlayerView {
        let videoView = FTStroyPlayerView()
        videoView.translatesAutoresizingMaskIntoConstraints = false
        videoView.tag = mediaIndex + snapViewTagIndicator
        ...
        NSLayoutConstraint.activate([
            videoView.leadingAnchor.constraint(equalTo: (mediaIndex == 0) ? scrollview.leadingAnchor : scrollview.subviews[previousSnapIndex].trailingAnchor),
        ...
        if(mediaIndex != 0) {
            NSLayoutConstraint.activate([
                videoView.leadingAnchor.constraint(equalTo: scrollview.leadingAnchor, constant: CGFloat(mediaIndex)*scrollview.width),
            ])
        }
        return videoView
    }
    private func getVideoView(with index: Int) -> FTStroyPlayerView? {
        if let videoView = scrollview.subviews.filter({$0.tag == index + snapViewTagIndicator}).first as? FTStroyPlayerView {
            return videoView
        }
        return nil
    }
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                switch result {
                    case .success(_):
                        ...
                        if(strongSelf.handpickedMediaIndex == strongSelf.mediaIndex && url == strongSelf.story!.mediaList?[strongSelf.mediaIndex].url) {
    ...
    private func showRetryButton(with url: String, for snapView: UIImageView) {
        self.retryBtn = FTRetryLoaderButton.init(withURL: url)
    ...
    private func startPlayer(videoView: FTStroyPlayerView, with url: String) {
        if scrollview.subviews.count > 0 {
            if story?.isCompletelyVisible == true {
                videoView.startAnimating()
                FTVideoCacheManager.shared.getFile(for: url) { [weak self] (result) in
                    guard let strongSelf = self else { return }
                    switch result {
                        case .success(let videoURL):
                            /// Start progressor only if handpickedSnapIndex matches with snapIndex
                            if(strongSelf.handpickedMediaIndex == strongSelf.mediaIndex) {
    ...
    @objc private func didTapSnap(_ sender: UITapGestureRecognizer) {
        ...
        if let snapCount = story?.mediaCount {
            var n = mediaIndex
            /*!
             * Based on the tap gesture(X) setting the direction to either forward or backward
             */
            if let snap = story?.mediaList?[n], snap.type == .image, getSnapview()?.image == nil {
            ...
            if touchLocation.x < scrollview.contentOffset.x + (scrollview.frame.width/2) {
                direction = .backward
                if mediaIndex >= 1 && mediaIndex <= snapCount {
                ...
                if mediaIndex >= 0 && mediaIndex <= snapCount {
    ...
    @objc private func didEnterForeground() {
        if let snap = story?.mediaList?[mediaIndex] {
            if snap.type == .video {
                let videoView = getVideoView(with: mediaIndex)
                startPlayer(videoView: videoView!, with: snap.url ?? "")
            }else {
                startSnapProgress(with: mediaIndex)
            }
        }
    }
    @objc private func didEnterBackground() {
        ...
        resetSnapProgressors(with: mediaIndex)
    }
    private func willMoveToPreviousOrNextSnap(n: Int) {
        if let count = story?.mediaCount {
            if n < count {
                //Move to next or previous snap based on index n
                let x = n.toFloat * frame.width
                let offset = CGPoint(x: x,y: 0)
                scrollview.setContentOffset(offset, animated: false)
                story?.lastPlayedMediaIndex = n
                handpickedMediaIndex = n
                mediaIndex = n
            } else {
    ...
    private func fillupLastPlayedSnap(_ sIndex: Int) {
        if let snap = story?.mediaList?[sIndex], snap.type == .video {
            videoMediaIndex = sIndex
            stopPlayer()
        }
    ...
    private func gearupTheProgressors(type: MediaType, playerView: FTStroyPlayerView? = nil) {
        if let holderView = getProgressIndicatorView(with: mediaIndex),
            let progressView = getProgressView(with: mediaIndex){
            progressView.story_identifier = self.story?.internalIdentifier
            progressView.snapIndex = mediaIndex
    ...
    func startProgressors() {
        DispatchQueue.main.async {
            if self.scrollview.subviews.count > 0 {
                let imageView = self.scrollview.subviews.filter{v in v.tag == self.mediaIndex + snapViewTagIndicator}.first as? UIImageView
                if imageView?.image != nil && self.story?.isCompletelyVisible == true {
                    ...
                    if self.story?.isCompletelyVisible == true {
                        let videoView = self.scrollview.subviews.filter{v in v.tag == self.mediaIndex + snapViewTagIndicator}.first as? FTStroyPlayerView
                        let snap = self.story?.mediaList?[self.mediaIndex]
    ...
    func getProgressView(with index: Int) -> FTStroyProcessView? {
        let progressView = storyHeaderView.getProgressView
        if progressView.subviews.count > 0 {
            let pv = getProgressIndicatorView(with: index)?.subviews.first as? FTStroyProcessView
    ...
    func getProgressIndicatorView(with index: Int) -> FTStoryProcessIndicatorView? {
        let progressView = storyHeaderView.getProgressView
        return progressView.subviews.filter({v in v.tag == index+progressIndicatorViewTag}).first as? FTStoryProcessIndicatorView ?? nil
    }
    ...
    func deleteSnap() {
        let progressView = storyHeaderView.getProgressView
        clearLastPlayedSnaps(mediaIndex)
        stopSnapProgressors(with: mediaIndex)
        
        let snapCount = story?.mediaCount ?? 0
         ...
         */
        if story?.mediaList?[mediaIndex].type == .video {
            stopPlayer()
        }
        scrollview.subviews.filter({$0.tag == mediaIndex + snapViewTagIndicator}).first?.removeFromSuperview()
...
//        story?.mediaList?[mediaIndex].isDeleted = true
        direction = .forward
        for sIndex in 0..<mediaIndex {
            if let holderView = self.getProgressIndicatorView(with: sIndex),
                let progressView = self.getProgressView(with: sIndex){
                progressView.widthConstraint?.isActive = false
                progressView.widthConstraint = progressView.widthAnchor.constraint(equalTo: holderView.widthAnchor, multiplier: 1.0)
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now()+0.3) {[weak self] in
            ...
            strongSelf.willMoveToPreviousOrNextSnap(n: strongSelf.mediaIndex)
        }
    ...
    public func willDisplayCellForZerothIndex(with sIndex: Int, handpickedSnapIndex: Int) {
        self.handpickedMediaIndex = handpickedSnapIndex
    ...
    public func stopPlayer() {
        let videoView = getVideoView(with: videoMediaIndex)
        if videoView?.player?.timeControlStatus != .playing {
            getVideoView(with: videoMediaIndex)?.player?.replaceCurrentItem(with: nil)
        }
        videoView?.stop()
    ...
    public func pauseEntireSnap() {
        let v = getProgressView(with: mediaIndex)
        let videoView = scrollview.subviews.filter{v in v.tag == mediaIndex + snapViewTagIndicator}.first as? FTStroyPlayerView
        if videoView != nil {
            v?.pause()
            videoView?.pause()
        }else {
    ...
    public func retryRequest(view: UIView, with url: String) {
        ...
        }else if let v = view as? FTStroyPlayerView {
            v.removeRetryButton()
...
extension FTStoryPreviewCell: FTStroyHeaderViewProtocol {
    func didTapCloseButton() {
...
extension FTStoryPreviewCell : FTRetryBtnDelegate {
...
extension FTStoryPreviewCell: FTPlayerObserver {
    
    func didStartPlaying() {
        if let videoView = getVideoView(with: mediaIndex), videoView.currentTime <= 0 {
                    ...
                    if let duration = videoView.currentItem?.asset.duration {
                        if Float(duration.value) > 0 {
                            progressView.start(with: duration.seconds, holderView: holderView, completion: {(identifier, snapIndex, isCancelledAbruptly) in
                                if isCancelledAbruptly == false {
                                    self.videoMediaIndex = snapIndex
                                    self.stopPlayer()
                                    self.didCompleteProgress()
                                } else {
                                    self.videoMediaIndex = snapIndex
    ...
    func didFailed(withError error: String, for url: URL?) {
        ...
        if let videoView = getVideoView(with: mediaIndex), let videoURL = url {
            self.retryBtn = FTRetryLoaderButton(withURL: videoURL.absoluteString)
            self.retryBtn.translatesAutoresizingMaskIntoConstraints = false
            self.retryBtn.delegate = self
            self.isUserInteractionEnabled = true
...
extension FTStoryPreviewCell: UIGestureRecognizerDelegate {
