//
//  FTCubeAttributesAnimator.swift
//  FTAnimatedCollectionLayout
//
//  Created by Journey on 27/11/2020.
...
public struct FTCubeAttributesAnimator: FTLayoutAttributesAnimator {
    ...
    public func animate(collectionView: UICollectionView, attributes: AnimatedCollectionViewLayoutAttributes) {
        ...
        if abs(position) >= 1 {
            ...
            attributes.contentView?.FTKeepCenterAndApplyAnchorPoint(CGPoint(x: position > 0 ? 0 : 1, y: 0.5))
        } else {
            let rotateAngle = totalAngle * position
            var transform = CATransform3DIdentity
            transform.m34 = perspective
            ...
            attributes.contentView?.FTKeepCenterAndApplyAnchorPoint(CGPoint(x: 0.5, y: position > 0 ? 0 : 1))
        }
    }
}
